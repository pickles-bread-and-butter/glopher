...
func removePluginManifestMapping(manifestMapping map[string]interface{}, pluginName *string) error {
  var err error = nil
  if pluginsMapping, ok := manifestMapping["Plugins"].(map[interface{}]interface{}); ok {
    if _, ok := pluginsMapping[*pluginName].(map[interface{}]interface{}); ok{
      log.Printf("Found plugin %s of proper type, removing.", *pluginName)
      delete(pluginsMapping, *pluginName)
    } else {
      log.Printf("Plugin %s not of right data type map[interface{}]interface{}, file map broken.", *pluginName)
    }
  } else {
    log.Println("Manifest object not map[interface{}]interface{}, file map broken.")
    err = errors.New("Manifest object not of type map[string]interface{}, file map broken")
  }
  return err
}
